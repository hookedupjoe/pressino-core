...
class ActAppCommon {
	...
	public static function setup_scripts($hook) {
		...
		$my_js_ver  = PRESSINO_PLUGIN_CORE_VERSION;
		...
		if( $screenbase == 'site-editor' || $screenbase == 'post' || $screenbase == 'page' ){
			wp_enqueue_script(
				...
				PRESSINO_PLUGIN_BLOCKS_URL . '/js/FixIFramePreviewInWP.js',
		...
		$tmpConfig = array(
			...
			'actappBaseURL'=> PRESSINO_PLUGIN_BASE_URL . '/', 
			'actappCatalogURL'=> PRESSINO_PLUGIN_BASE_URL . '/catalog/', 
			'libCatalogsURL'=> PRESSINO_PLUGIN_BASE_URL . '/core/catalogs/', 
			'restPath' => esc_url_raw( rest_url() ),
			'currentUser' => get_current_user_id(),
			'nonce' => wp_create_nonce( 'wp_rest' )
		);
		$tmpCats = array(
			'actapp' => PRESSINO_PLUGIN_BASE_URL . '/catalog/',
			'common' => PRESSINO_PLUGIN_BASE_URL . '/core/catalogs/common/', 
		);
		$tmpJson = json_encode($tmpConfig);
		$tmpScript = 'window.ActionAppCore.ActAppWP = ' . $tmpJson . ';';			
		$tmpScript .= 'ActionAppCore.dir.catalogs._wordpress = "' . PRESSINO_PLUGIN_BASE_URL . '/catalog/' . '";';
		$tmpScript .= 'ActionAppCore.dir.catalogs._common = "' . PRESSINO_PLUGIN_BASE_URL . '/core/catalogs/common/' . '";';
