...
class ActAppDesigner {
	...
	public static function getDataVersion(){
		return PRESSINO_PLUGIN_CORE_VERSION;
	}
	...
	public static function loadStandardBlock($theName, $theFileName = '', $theDependencies = null){
		...
		wp_enqueue_script(
			$theName, 
			PRESSINO_PLUGIN_URL . '/blocks/actappdesign/' . $tmpFN . '.js',
			$tmpDepDefaults,
			true
		);
	...
	public static function override_tpl($template){
		...
		if( $tmpIsDeveloperView == 'fields' && current_user_can('actappdesign') ){
			$template = PRESSINO_PLUGIN_DIR . '/tpl/actappdesigndoc.php';
			return $template;
		}
		
		$post_types = array( 'actappdesign' );
		if ( is_singular( $post_types ) && $pagename != '' && file_exists( PRESSINO_PLUGIN_DIR . '/tpl/designer-'.$pagename.'.php' ) ){
			$template = PRESSINO_PLUGIN_DIR . '/tpl/designer-'.$pagename.'.php';
			return $template;
		}
		if ( is_singular( $post_types ) && file_exists( PRESSINO_PLUGIN_DIR . '/tpl/designer.php' ) ){
			$template = PRESSINO_PLUGIN_DIR . '/tpl/designer.php';
			return $template;
		}

		$post_types = array( 'actappdoc' );
		if ( is_singular( $post_types ) && file_exists( PRESSINO_PLUGIN_DIR . '/tpl/actappdoc.php' ) ){
			$template = PRESSINO_PLUGIN_DIR . '/tpl/actappdoc.php';
			return $template;
		}

		$post_types = array( 'actappdesigndoc');
		if ( is_singular( $post_types ) && file_exists( PRESSINO_PLUGIN_DIR . '/tpl/actappdesigndoc.php' ) ){
			$template = PRESSINO_PLUGIN_DIR . '/tpl/actappdesigndoc.php';
			return $template;
		}
		$post_types = array( 'actappelem' );
		if ( is_singular( $post_types ) && file_exists( PRESSINO_PLUGIN_DIR . '/tpl/actappdoc.php' ) ){
			$template = PRESSINO_PLUGIN_DIR . '/tpl/actappelem.php';
			return $template;
		}
	...
	public static function actapp_init_admin_scripts(){
		// wp_register_style( 'aa-core-admin_css',   PRESSINO_PLUGIN_URL . '/css/wp-admin.css', false,  $my_css_ver );
		// wp_enqueue_style ( 'aa-core-admin_css' );
	}
	
	public static function actapp_init_blocks($theHook) {
		$my_css_ver = '1'; //todo
		wp_register_style( 'act-app-designer_css',   PRESSINO_PLUGIN_URL . '/css/designer.css', false,  $my_css_ver );
		//--- Load the action app core components and ActionAppCore.common.blocks add on
		wp_enqueue_script(
			'actapp-core-blocks', 
			PRESSINO_PLUGIN_URL . '/js/DesignerDashboard.js',
	...
	public static function baseDir() {
		return PRESSINO_PLUGIN_DIR;
	}
	public static function baseURL() {
		return PRESSINO_PLUGIN_URL;
	}
	...
	public static function showApplications(){
		...
		include(PRESSINO_PLUGIN_DIR . '/tpl/designer-apps.php');
	}
	public static function showDesigner(){
		//esc_html_e( 'showDesigner', 'textdomain' );
		include(PRESSINO_PLUGIN_DIR . '/tpl/designer-ui.php');
	}
...
//require_once PRESSINO_PLUGIN_BLOCKS_DIR . '/blocks/actappdesign/ActAppReportView/Object.php';

//--- This demo shows backend data based on the current user
//require_once PRESSINO_PLUGIN_BLOCKS_DIR . '/blocks/actappdesign/ActAppDynoUserInfo/Object.php';
