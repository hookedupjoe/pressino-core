...
//==========================================
	...
	public function get_page_code($request) {
		$tmpRoot = PRESSINO_PLUGIN_DIR . '/apps/DevDesigner/app/catalog/designer/panels';
...
// //		self::actapp_unset_post_lock($body->doc->ID);
	...
	public function get_json_from_csv_file($request) {
		...
		$file = PRESSINO_PLUGIN_URL . '/extra/DepthChartPreWeek7.csv';
		...
		$fn = $_GET['fn'];
		// if(!empty($fn)){
		// 	$file = PRESSINO_PLUGIN_URL . '/extra/'. $fn;
		// }
		$tmpStart = 1;
		$tmpEnd = $tmpDocCount;		
		if( $pos != ''){
	...
	public function get_json_from_csv($request) {
		//$file="C:\\aa\\mock-data.csv";
		$file = PRESSINO_PLUGIN_URL . '/extra/mock-data.csv';
		$csv = file_get_contents($file);
		$array = array_map("str_getcsv", explode("\n", $csv));
		$tmpDocCount = count($array);
